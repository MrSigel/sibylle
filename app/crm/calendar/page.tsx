...
export default function CalendarPage() {
  ...
  async function fetchData() {
    ...
    const [{ data: appts, error: apptError }, { data: custs }] = await Promise.all([
    ...
    if (apptError) {
      ...
      setError("Termine konnten nicht geladen werden. Bitte prüfe die Verbindung.");
    }
  ...
  function openEditAppointment(appointment: any) {
    ...
    setFormAppointment({
      ...
      start_time: formatDateTimeLocal(new Date(appointment.start_time)),
      end_time: appointment.end_time ? formatDateTimeLocal(new Date(appointment.end_time)) : "",
      appointment_type: appointment.appointment_type || "Zoom",
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.form onSubmit={saveAppointment} initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} className="relative w-full max-w-xl rounded-[32px] bg-white p-8 shadow-2xl md:p-12">
              <h2 className="mb-8 text-2xl font-bold text-warmBlack">{editingId ? "Termin bearbeiten" : "Termin anlegen"}</h2>
              ...
              <div className="mt-8 flex gap-4">
                <button type="button" onClick={() => { setIsModalOpen(false); setEditingId(null); }} className="flex-1 rounded-full border border-gold/20 py-4 font-bold text-deepGold hover:bg-gold/5">Abbrechen</button>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {loading ? (
              <div className="rounded-2xl border border-gold/10 bg-white p-10 text-center text-sm italic text-deepGold/40">Termine werden geladen...</div>
              ...
              <motion.div key={apt.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }} className="group relative flex gap-4 rounded-2xl border border-gold/15 bg-white p-5 shadow-soft hover:border-gold/30">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    <button onClick={() => openEditAppointment(apt)} className="text-[10px] font-bold uppercase tracking-widest text-deepGold hover:text-gold">Bearbeiten</button>
