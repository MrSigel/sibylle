...
export default function SelbsttestAdminPage() {
  ...
  return (
    <main className="min-h-screen bg-mist/20 p-6 lg:p-10">
        ...
        <div className="flex flex-col gap-4 rounded-[32px] border border-gold/15 bg-white p-8 shadow-soft sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-5">
            <div className="relative h-12 w-36">
              <Image src="/assets/sibylle/brand/logo-header.png" alt="Sibylle Bergold" fill className="object-contain object-left" />
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-softGold">Admin CRM</p>
              <h1 className="text-3xl font-bold text-warmBlack">Beziehungs-Kompass Leads</h1>
              <p className="mt-1 text-sm text-deepGold/70">Alle eingegangenen Selbsttest-Anfragen chronologisch sortiert.</p>
            </div>
          </div>
          <Link href="/crm" className="rounded-full border border-gold/20 px-5 py-3 text-sm font-bold text-deepGold hover:bg-gold/5">Zurück ins CRM</Link>
        </div>

        <div className="grid gap-6 md:grid-cols-3">
          <Stat label="Leads gesamt" value={String(leads.length)} />
          <Stat label="Heute" value={String(leads.filter((lead) => new Date(lead.created_at).toDateString() === new Date().toDateString()).length)} />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gold/5">
                {loading ? (
                  <tr><td colSpan={4} className="px-8 py-10 text-center text-sm italic text-deepGold/40">Daten werden geladen...</td></tr>
                ) : leads.length === 0 ? (
                  <tr><td colSpan={4} className="px-8 py-10 text-center text-sm italic text-deepGold/40">Noch keine Selbsttest-Leads vorhanden.</td></tr>
                  ...
                  <motion.tr key={lead.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.025 }} className="hover:bg-gold/5">
                    ...
                    <td className="px-8 py-5"><span className="rounded-full bg-sand/40 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-deepGold">{lead.ergebnis_typ}</span></td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
