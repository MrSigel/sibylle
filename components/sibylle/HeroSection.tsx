        ...
        <div className="grid items-center gap-16 lg:grid-cols-[1fr_.9fr] lg:gap-24">
          <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: .15 } } }}>
            ...
            <motion.h1 variants={{ hidden:{opacity:0,y:30},show:{opacity:1,y:0,transition:{duration:1,ease}} }} className="editorial mt-7 max-w-[720px] text-[clamp(2.8rem,5.8vw,6.2rem)] leading-[.9] text-warmBlack">
              Wenn dein Leben <span className="italic text-deepGold">wiederholt,</span> will etwas gesehen werden.
            </motion.h1>
            <motion.p variants={{ hidden:{opacity:0,y:20},show:{opacity:1,y:0,transition:{duration:.8,ease}} }} className="mt-8 max-w-lg text-lg leading-8 text-deepGold/85 md:text-xl">
            ...
            <motion.div variants={{ hidden:{opacity:0},show:{opacity:1,transition:{delay:1.2,duration:1}} }} className="mt-14 flex items-center gap-6 border-t border-gold/10 pt-10">
              <div className="flex -space-x-4">
                ...
                <div className="relative flex h-14 w-14 items-center justify-center rounded-full border-4 border-white bg-softGold font-bold text-white shadow-md text-sm">
                  +30
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <p className="text-[.95rem] font-bold text-deepGold tracking-tight">30+ 5-Sterne Bewertungen auf Google</p>
              </div>
            </motion.div>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <motion.div 
                ...
                className="absolute inset-y-0 left-0 z-10 w-1/2 bg-white/40 backdrop-blur-md border-r border-white/20 pointer-events-none"
              />
              <motion.div 
                style={{ x: rightPanelX, opacity: panelOpacity }}
                className="absolute inset-y-0 right-0 z-10 w-1/2 bg-white/40 backdrop-blur-md border-l border-white/20 pointer-events-none"
              />
            </motion.div>
        ...
        <motion.div initial="hidden" animate="show" variants={{hidden:{},show:{transition:{delayChildren:1.4,staggerChildren:.15}}}} className="mt-24 grid gap-5 md:mt-32 md:grid-cols-3">
          {heroQuestions.map((question,index)=><motion.div key={question} variants={{hidden:{opacity:0,y:30},show:{opacity:1,y:0,transition:{duration:.8,ease}}}} whileHover={{y:-12,borderColor:'rgba(198,168,114,.4)',boxShadow:'0 20px 40px rgba(35,42,26,0.08)'}} className="premium-panel group cursor-default rounded-[2.5rem] p-8 transition-all duration-500 bg-white/40">
            <div className="flex flex-col gap-6">
              ...
              <p className="text-xl leading-8 text-warmBlack group-hover:text-deepGold transition-colors duration-500">{question}</p>
            </div>
          </motion.div>)}
        </motion.div>
      </div>
