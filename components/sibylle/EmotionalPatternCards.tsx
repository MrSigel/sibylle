...
export function EmotionalPatternCards() {
  return <section className="grain section-shell overflow-hidden bg-white text-warmBlack">
    <div className="absolute left-[15%] top-0 h-[40rem] w-[40rem] rounded-full bg-softGold/10 blur-[130px]" />
    <div className="container relative z-10 grid gap-16 lg:grid-cols-[.85fr_1.15fr] lg:items-center lg:gap-24">
      <motion.div initial={{opacity:0,x:-35}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{duration:1,ease:[0.22, 1, 0.36, 1]}}>
        ...
        <div className="mt-12">
           <CTAButton href="/beziehungsmuster" variant="secondary">Mehr über Muster erfahren</CTAButton>
        </div>
      </motion.div>
          ...
          <motion.article 
            ...
            variants={{
              ...
              hover: { 
                ...
                borderColor: "rgba(132, 103, 51, 0.2)",
                transition: { duration: 0.4, ease: [0.22, 1, 0.36, 1] }
              }
            }}
            className="group relative rounded-[2.2rem] border border-gold/15 bg-white shadow-[0_15px_45px_rgba(132,103,51,0.04)] p-8 pl-20 backdrop-blur-md transition-all duration-500 md:p-10 md:pl-28"
          >
            <motion.span 
              variants={{
                initial: { x: 0, scale: 1 },
            ...
            <h3 className="editorial text-3xl md:text-4xl text-warmBlack">{item.title}</h3>
            <p className="mt-4 max-w-lg text-base leading-8 text-deepGold/60 transition-colors duration-500 group-hover:text-deepGold/80">
              {item.description}
            </p>
